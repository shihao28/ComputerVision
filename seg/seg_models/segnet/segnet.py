...
class SegNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, pretrained=True, freeze_bn=False, **_):
        ...
        self._initialize_weights(
            self.stage1_decoder, self.stage2_decoder, self.stage3_decoder,
...
class SegResNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, backbone='resnet50', pretrained=True, freeze_bn=False, **_):
        ...
        resnet_blocks = list(resnet.children())[4:-2]
        self.encoder = nn.Sequential(*resnet_blocks)

        # Decoder
        resnet_untrained = getattr(models, backbone)(pretrained=True)
        resnet_blocks = list(resnet_untrained.children())[4:-2][::-1]
        decoder = []
...
if __name__ == "__main__":
    SegResNet(num_classes=5, pretrained=False)
